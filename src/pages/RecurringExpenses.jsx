...
export default function RecurringExpenses() {
  ...
  async function load() {
    ...
    const [{ data: tx }, { data: dis }] = await Promise.all([
      supabase.from('transactions').select('id, date, description, amount').eq('type', 'utgift').order('date', { ascending: false }).limit(1000),
      ...
      {!loading && detected.length > 0 && (
        <div className="two-col" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))' }}>
          <div className="card card-pad">
            <div className="stat-label">Totalt per måned</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 600 }}>{formatKr(totalMonthly)}</div>
          </div>
          <div className="card card-pad">
            <div className="stat-label">Totalt per år</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 600 }}>{formatKr(totalMonthly * 12)}</div>
          </div>
          <div className="card card-pad">
            <div className="stat-label">Antall</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 600 }}>{detected.length}</div>
          </div>
        </div>
      )}
          ...
          detected.map((d) => (
            <div key={d.vendorKey} className="row-between" style={{ padding: 'var(--space-3) var(--space-4)', borderBottom: '1px solid var(--border)' }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div className="text-muted" style={{ fontSize: 12 }}>{d.occurrences} transaksjoner over {d.monthCount} måneder · sist {formatDate(d.lastDate)}</div>
              </div>
              <div className="row">
                <span className="amount-negative" style={{ fontWeight: 600 }}>−{formatKr(d.monthlyEstimate)}/mnd</span>
